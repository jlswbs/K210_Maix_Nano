// Stereo Audio PWM - Hopf oscillator example //
...
int audio_callback(void *ctx){
  ...
  x = nx + dt * (-ny + nx * (p - (powf(nx, 2.0f) + powf(ny, 2.0f))));
  y = ny + dt * (nx + ny * (p - (powf(nx, 2.0f) + powf(ny, 2.0f))));
     
  double duty_l = 0.48f + (1.3f * x);
  double duty_r = 0.48f + (1.3f * y);
